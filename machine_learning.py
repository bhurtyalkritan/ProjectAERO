...
class DroneMLManager:
    ...
    def update_experience(self, drone_id, outcome, conditions, cost):
        """
        Updates Q-value based on outcome (success/fail), cost, and discounting.
        A success yields a positive reward inversely related to cost.
        A failure yields a negative reward.
        """
        state = self.get_state_key(drone_id, conditions)
        old_q = self.q_table.get(state, 1.0)
        if outcome == "success":
            reward = max(1, 2000 / (cost + 1))  # bigger reward if cost is small
        else:
            reward = -50
